...
\setbeamertemplate{footline}[text line]{%
  \parbox{\linewidth}{\vspace*{-10pt}NANOx81-Data Science in Materials Science\hfill\insertpagenumber}}
...
\begin{frame}{Model Fitting Procedure}
    ...
    \begin{figure}
        \begin{tikzpicture}
        [
            rectnode/.style={rectangle, rounded corners, minimum width=3cm, minimum height=2cm,text centered, draw=black, fill=green!30},
            invisiblenode/.style={circle,radius=0},
            node distance=1cm,
        ]
%Nodes

            \node[rectnode] (node1) [text width=3.5cm]{\footnotesize{Train-test split, e.g., 80:20}};
            \node[rectnode] (node2) [right=of node1, text width=3.5cm]{\footnotesize{Fit scaler / transforms on \textbf{training} data (if needed)}};
            \node[rectnode] (node3) [right=of node2, text width=3.5cm] {\footnotesize{CV or train/val using \textbf{training} data only}};
            \node[rectnode] (node4) [below=of node3, text width=3.5cm] {\footnotesize{Select best param. set using CV / val scores.}};
           \node[rectnode] (node5) [left=of node4, text width=3.5cm] {\footnotesize{Refit using best param. set and \textbf{training} data}};
           \node[rectnode] (node6) [left=of node5, text width=3.5cm] {\footnotesize{Evaluate using \textbf{test} set}};


            \draw[->, very thick] (node1.east) -- (node2.west);
            \draw[->, very thick] (node2.east) -- (node3.west);            \draw[->, very thick] (node3.south) -- (node4.north);            \draw[->, very thick] (node4.west) -- (node5.east);  
            \draw[->, very thick] (node5.west) -- (node6.east);  
...
\begin{frame}{Loss functions for regression}
    \begin{itemize}
        ...
        \item Other criteria include the Huber loss, which is also more robust against outliers:
        \begin{equation*}
